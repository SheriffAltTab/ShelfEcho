...
**Сайт задеплоєний через Google Cloud.**  
...
- **Фронт** — статична збірка (Vite): збирається в `dist/`, файли роздаються через хостинг (у вашому випадку інфраструктура Google Cloud). Домен shelfecho.site вказує на цей фронт.
...
Після розгортання бекенду на продакшені в конфігурації фронту (збірка для Google Cloud) потрібно вказати змінну `VITE_API_URL` на URL вашого API (наприклад `https://api.shelfecho.site` або окремий домен/сервіс), щоб логін, реєстрація та всі запити йшли на живий бекенд.
